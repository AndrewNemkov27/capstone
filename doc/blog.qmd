...
Below is a short demonstration of the process of the PaddleOCR model I utilized in my project as it is given an image and it identifies and extracts Japanese text from a Baki manga panel. Later proper cleaning and calculations are performed with these extracted phrases as all the collected information is recorded in the dataset.
...
The process begins with an image input. In this example, I used a simple image with few words for simplicity called DL-Raw.Net_48 (2).jpg. The OCR model runs its machine learning algorithm and identifies the text seen in the bright red box.
...
The process continues as this phrase is formatted to be computer readable and the model continues to see if it can identify any more text. When the model can no longer read any more information from the image, a numered success message is given in the output above.
...
The identified information of the DL-Raw.Net_48 (2).jpg image are input into the test dataset and many steps of data preperation are calculated and obtained from the extracted phrase(s).
...
To better understand the impact of themes on the story of Baki, I decided to visualize the most common nouns present across all three manga series in the form of a three side-by-side bar graphs. I began by cleaning my dataset to get a fair result of my visualization. The three main things I did are shown below:

**Stopword Removal:**
- I removed the dataset of "stopwords". An example of a common stopword is "the" or "be". They are called this because they "stop" or are excluded in almost all natural language processing. They provide no significant meaning and appear very frequently, potentially swaying the results of word analysis if they are not filtered out. In the beginning of the code section above you can see a large list of common stopwords I excluded from my dataset for this graph.

**Additional Cleaning:**
I continued by filtering my dataframe of this graph to only include words marked as nouns, since these will provide me with the most amount of context and meaninful information to understand the themes of Grappler Baki. I also removed words with spacing or numbers, as these can cause issues for proper visualization understanding.
...
The results of this graph are really interesting in that the word "Baki", the name of the main character in *Grappler Baki*, is one of the most often occuring nouns in all three series. In addition, words such as "Sumo", "body", "fighting", "strongest", "warrior", "death", "man" all clearly represent the themes on which the story of Grappler Baki stands upon. This relationship between noun frequency and thematic representation portrays the Japanese manga as one that is character-centric, as in the story would not look the same with totally different characters.
...
I continued my thematic analysis of Baki by visualizing another bar graph of figure 2.2. This one differs from figure 2.1 in that it visulizes the normalized distribution of parts of speech across the three Baki series used in my dataset. When I say normalized, I mean that adjusted to take into consideration of the proportion of parts of speech across series instead of just total number of occurrences.

The result of the above figure surprised me greatly, as the distribution of parts of speech across the series is nearly identical, with nouns, unknown, and verbs dominating the total proportion. This means that since 2005 (the year when the series of Hanma Baki began) till today's year of 2025, the way that *Grappler Baki* is written has not changed. This means that the author still works and desires to expand on the story of Baki, following the same narrative style and word usage. This also means that the themes of Baki have not changed over the past 20 years
...
The final visualization of figure 2.3 shows a the results of TF-IDF analysis on the dataset in a combined bar graph format. The term TF-IDF written out fully reads as term frequency-inverse document frequency and this is a technique in data analysis to measure the importance of words in a given document. During this process, a certain weight is applied to each word in a given dataset, meaning "heavier" words hold more meaning while "lighter" words hold less meaning. Weight of a word decreases by how frequently it appears in the overall word list. In general TF-IDF highlights words that are distinctive rather than common.

In the above visualization of the top 20 most important words in each Baki series is colored with a red hue. The graph clearly shows a similarity to the result in figure 2.1 of the most common nouns in all three Baki manga series. Names and words relating to the themes of the Grappler Baki story are shown, representing that characters are central to the story of Baki.
